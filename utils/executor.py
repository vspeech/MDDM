...
from xspeech.utils.train_utils import (wenet_join, batch_forward, batch_backward,
...
class Executor:
    ...
    def train(self, model, optimizer, scheduler, train_data_loader,
              ...
              #cv_data_loader, configs, scaler, group_join):
        ''' Train one epoch
        '''
        model.train()
        ...
        with model_context():
            for batch_idx, batch_dict in enumerate(train_data_loader):
                ...
                if batch_dict["pcm_lengths"].size(0) == 0:
                    continue
    ...
    def cv(self, model, cv_data_loader, configs):
        ...
        with torch.no_grad():
            for batch_idx, batch_dict in enumerate(cv_data_loader):
                ...
                info_dict["step"] = self.step
                info_dict["batch_idx"] = batch_idx
                info_dict["log_interval"] = 20

                num_utts = batch_dict["pcm_lengths"].size(0)
                if num_utts == 0:
                    continue

                info_dict = batch_forward(model, batch_dict, None, info_dict)
                loss = info_dict['loss_dict']['loss']
                pesq = info_dict['loss_dict']['avg_sisdr']
                stoi = info_dict['loss_dict']['avg_stoi']

                if torch.isfinite(loss):
                    num_seen_utts += num_utts
                    total_loss += loss.item() * num_utts
                    total_pesq += pesq.item() * num_utts
                    total_stoi += stoi.item() * num_utts

                info_dict["history_loss"] = total_loss / num_seen_utts
                info_dict["history_sisdr"] = total_pesq / num_seen_utts
                info_dict["history_stoi"] = total_stoi / num_seen_utts
                log_per_step(writer=None, info_dict=info_dict)
