...
from xspeech.dataio.dataset import Dataset
from xspeech.utils.checkpoint import save_checkpoint
from xspeech.utils.scheduler import WarmupLR, NoamHoldAnnealing
...
def add_dataset_args(parser):
    ...
    #parser.add_argument('--rir_data', required=True, help='rir data list')
    #parser.add_argument('--noise_data', required=True, help='noise data list')
...
def check_modify_and_save_config(args, configs):
    ...
    if 'fft_conf' in configs['dataset_conf']:
        input_dim = configs['dataset_conf']['fft_conf']['nfft'] // 2 + 1
    ...
    if int(os.environ.get('RANK', 0)) == 0:
        saved_config_path = os.path.join(configs['model_dir'], 'train.yaml')
...
def init_optimizer_and_scheduler(args, configs, model):
    if configs['optim'] == 'adam':
        optimizer = optim.Adam(model.parameters(), lr=configs['optim_conf']['lr'], betas=(configs['optim_conf']['momentum'], configs['optim_conf']['beta2']), weight_decay=configs['optim_conf']['weight_decay'], eps=1e-8)
...
def trace_and_print_model(args, model):
    ...
    if int(os.environ.get('RANK', 0)) == 0:
        if args.jit:
            ...
            script_model.save(os.path.join(configs['model_dir'], 'init.zip'))
        if args.print_model:
            print(model)
...
def init_summarywriter(args, configs):
    writer = None
    if int(os.environ.get('RANK', 0)) == 0:
        os.makedirs(configs['model_dir'], exist_ok=True)
        exp_id = os.path.basename(configs['model_dir'])
...
def log_per_step(writer, info_dict):
    ...
    history_sisdr = info_dict.get("history_sisdr", 0.0)
    history_stoi = info_dict.get("history_stoi", 0.0)
    ...
    if (batch_idx + 1) % log_interval == 0:
        ...
        if tag == "TRAIN":
            ...
            log_str += 'history sisdr {:.6f} rank {} '.format(history_sisdr, rank)
            log_str += 'history stoi {:.6f} rank {}'.format(history_stoi, rank)
