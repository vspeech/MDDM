...
from mydemucs.attention import MultiHeadedAttention
...
class MyTransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        self.attn: nn.Module
        self.attn = MultiHeadedAttention(d_model, d_hidden, nhead, dropout=dropout)
        ...
        if sparse:
            self.self_attn = MultiheadAttention(
                d_model, d_hidden, nhead, dropout=dropout, batch_first=batch_first,
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        if self.norm_first:
            ...
            x = self.norm1(
                x + self.gamma_1(self._sa_block(x, x, src_mask))
    ...
    def _sa_block(self, q, k, attn_mask=None):
        x = self.attn(q, k, k, attn_mask=attn_mask)
...
class CrossTransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        self.cross_attn = MultiHeadedAttention(d_model, d_hidden, nhead, dropout=dropout)
        # Implementation of Feedforward model
        self.linear1 = nn.Linear(d_model, dim_feedforward, **factory_kwargs)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(dim_feedforward, d_model, **factory_kwargs)
    ...
    def _ca_block(self, q, k, attn_mask=None):
        ...
        x = self.cross_attn(q, k, k, attn_mask=attn_mask)
        return self.dropout1(x)
...
class MultiheadAttention(nn.Module):
    def __init__(
        ...
        self.q = torch.nn.Linear(embed_dim, hidden_dim, bias=bias)
        self.k = torch.nn.Linear(embed_dim, hidden_dim, bias=bias)
