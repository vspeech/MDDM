...
from xspeech.module.masked_modules import MaskedBatchNorm1d, MaskedGroupNorm
from mydemucs.token.Hyperparameters import Hyperparameters as hp
...
class NTL(nn.Module):
    ...
    def __init__(self, out_dim):
        ...
        if hp.E != out_dim:
            self.out = nn.Linear(hp.E, out_dim, bias=False)

    def forward(self, inputs, lengths):
        enc_out = self.encoder(inputs, lengths)
        style_embed = self.stl(enc_out, lengths)
        if hp.E != self.out_dim:
            style_embed = self.out(style_embed)
        style_embed = style_embed.unsqueeze(1).permute(0, 1, 3, 2)
        return style_embed
...
class ReferenceEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        convs = [nn.Conv2d(in_channels=filters[i],
                           ...
                           kernel_size=(3, 8),
                           stride=(1, 4),
                           padding=(1, 2)) for i in range(2)]
        convs1 = [nn.Conv2d(in_channels=filters[i + 2],
                           out_channels=filters[i + 3],
                           kernel_size=(3, 8),
                           stride=(1, 4),
                           padding=(1, 2)) for i in range(K-2)]
        convs.extend(convs1) 
        self.convs = nn.ModuleList(convs)
        size = [4, 16, 64, 256]
        #self.bns = nn.ModuleList([nn.BatchNorm2d(num_features=hp.ref_enc_filters[i]) for i in range(K)])
        self.bns = nn.ModuleList([MaskedBatchNorm1d(num_features=hp.ref_enc_filters[i] * 2048 // size[i]) for i in range(K)])

        out_channels = self.calculate_channels(hp.n_mels, [8, 8, 8, 8], [4, 4, 4, 4], [2, 2, 2, 2], K)
    ...
    def forward(self, inputs, lengths):
        N = inputs.size(0)
        out = inputs.view(N, 1, hp.n_mels, -1).permute(0, 1, 3, 2)  # [N, 1, Ty, n_mels]
        ...
        packed_inputs = pack_padded_sequence(out, lengths.cpu(), enforce_sorted=False, batch_first=True)
        #self.gru.flatten_parameters()
        memory = self.gru(packed_inputs)[0]  # out --- [1, N, E//2]
        packed_inputs, _ = pad_packed_sequence(memory, batch_first=True)
        #return memory   #[N, T, 2 * E//2]
        return packed_inputs

    def calculate_channels(self, L, kernel_size, stride, pad, n_convs):
        for i in range(n_convs):
            L = (L - kernel_size[i] + 2 * pad[i]) // stride[i] + 1
        return L
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, lengths=None):
        ...
        if lengths is not None:
            mask = make_pad_mask(lengths).unsqueeze(-1)
