...
from mydemucs.utils import center_trim, unfold
from mydemucs.transformer import LayerScale
...
class DConv(nn.Module):
    ...
    def __init__(self, channels: int, compress: float = 4, depth: int = 2, init: float = 1e-4,
        ...
        for d in range(self.depth):
            ...
            layer1 = nn.Sequential(*mods1)
            self.layers1.append(layer1)
            if attn:
                self.ls = LocalState(hidden, heads=heads, ndecay=ndecay)
            if lstm:
                self.blstm = BLSTM(hidden, layers=2, max_steps=200, skip=True)

    def forward(self, x, mask):
        for layer, layer1 in zip(self.layers, self.layers1):
            ori = x
            #x = x + layer(x)
            x = layer(x)
            if mask != None:
...
class LocalState(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if mask != None:
            mask = ~mask.unsqueeze(-1).eq(0)  # (batch, 1, T, *)
            dots = dots.masked_fill(mask, -float('inf'))
            weights = torch.softmax(dots, dim=2).masked_fill(
                    mask, 0.0)  # (batch, head, time2, time1)
        else:
