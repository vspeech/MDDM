...
import xspeech.dataio.processor as processor
...
def Dataset(data_type,
    ...
    add_rir = conf.get('add_rir', True)
    if add_rir:
        rir_conf = conf.get('rir_conf', {})
        rir_list_file = rir_conf['rir_list_file']
        rir_lists = read_lists(rir_list_file)
        rir_conf['rir_lists'] = rir_lists
        dataset = Processor(dataset, processor.reverb_rir, **rir_conf)

    add_noise = conf.get('add_noise', True)
    if add_noise:
        noise_conf = conf.get('noise_conf', {})
        noise_list_file = noise_conf['noise_list_file']
        noise_lists = read_lists(noise_list_file)
        noise_conf['noise_lists'] = noise_lists
        dataset = Processor(dataset, processor.add_noise, **noise_conf)
   
    feats_type = conf.get('feats_type', 'fft')
    assert feats_type in ['fft', 'fbank', 'mfcc', 'log_mel_spectrogram', 'none']
    if feats_type == 'fbank':
        ...
        fft_conf = conf.get('fft_conf', {})
        dataset = Processor(dataset, processor.compute_fft, **fft_conf)
    elif feats_type == 'none':
        pass
    ...
    padding = conf.get('padding', True)
    if padding:
        dataset = Processor(dataset, processor.padding)
    else:
        dataset = Processor(dataset, processor.padding_pcm)
    return dataset
