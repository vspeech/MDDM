...
from xspeech.se.se_model import init_se_model
from xspeech.utils.executor import Executor
from xspeech.utils.config import override_config
from xspeech.utils.train_utils import (
...
def main():
    ...
    assert 'model_dir' in configs
    if not os.path.isdir(configs['model_dir']):
    ...
    train_dataset, train_data_loader = init_dataset_and_dataloader(args, configs)
    ...
    model, configs = init_se_model(configs)
